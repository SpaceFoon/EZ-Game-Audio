...
export const searchFiles = async (filePath, inputFormats) => {
  try {
    const fileExtensions = inputFormats?.length
      ? inputFormats
      : ["mp3", "wav", "flac"];
        ...
        await new Promise((resolve, reject) => {
          ...
          ffmpegCommand.on("close", (data) => {
            if (data.code === 0) {
              ...
              console.error(
                `Conversion failed for ${getFilename(
                  inputFile
                )} to ${getFilename(outputFile)}. Exit code: ${data.code}`
              );
              retryCount++;
              console.log(`Retrying... (Attempt ${retryCount}/${maxRetries})`);
              reject(
                new Error(
                  `Conversion failed for ${inputFile}. Exit code: ${data.code}`
                )
              );
            }
          });
          ...
          ffmpegCommand.on("error", (err) => {
            console.error(
              `Error during conversion for ${getFilename(
                inputFile
              )} to ${getFilename(outputFile)}. Retrying...`
            );
            retryCount++;
            console.log(`Retrying... (Attempt ${retryCount}/${maxRetries})`);
            reject(err);
